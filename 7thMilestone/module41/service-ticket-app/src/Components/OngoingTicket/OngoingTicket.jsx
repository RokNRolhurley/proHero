import React, { use } from 'react';

const OngoingTicket = ({ ticketPromise }) => {

    const ticketData = use(ticketPromise);
    // console.log(ticketData);
    const ongoingCount = ticketData.filter(ticket => ticket.status === 'In Progress' ).length;
    const resolvedCount = ticketData.filter(ticket => ticket.status === 'Closed').length;

    return (
        <div className='max-w-[1200px] mx-auto flex justify-between items-center px-8 py-8'>
        
            <div className='bg-[#422AD5] p-4 rounded-lg shadow-md h-[250px] w-[550px] flex flex-col justify-center items-center text-white'> 
                <h1 className='text-2xl font-bold mb-4 '>Ongoing Tickets</h1>
                <span className='text-2xl font-bold mb-4'>{ongoingCount}</span>
            </div>
            <div className='bg-[#422AD5] p-4 rounded-lg shadow-md h-[250px] w-[550px] flex flex-col justify-center items-center text-white'> 
                <h1 className='text-2xl font-bold mb-4'>Resolved Tickets</h1>
                <span className='text-2xl font-bold mb-4'>{resolvedCount}</span>  
            </div>
        </div>
    
    );
};

export default OngoingTicket;